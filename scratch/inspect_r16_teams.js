...
worldcupJson.matches.forEach(m => {
  ...
  const id = `KO-${stage}-${stageCounts[stage]}`;
  MATCH_NUM_TO_ID[m.num] = id;
  MATCH_ID_TO_NUM[id] = m.num;
});
...
async function run() {
  ...
  jsonKnockouts.filter(m => m.round === 'Round of 16').forEach(jm => {
    const res = resolvedByNum[jm.num];
    console.log(`Match ${jm.num} (${MATCH_NUM_TO_ID[jm.num]}):
    Original: ${jm.team1} vs ${jm.team2}
    Resolved: ${TEAMS[res?.home]?.name || res?.home} (${res?.home}) vs ${TEAMS[res?.away]?.name || res?.away} (${res?.away})`);
  });
}
